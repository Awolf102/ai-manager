...
- **2026-06-28** — **Cycle U1 (Destructive-action guardrails) ✅ MERGED to main** (merge `89373f7`). `deleteAgent` soft-deletes the agent folder to `.ai-manager/.trash/<slug>-<ts>/` (recoverable `memory.md`) instead of `fs.rm`; reusable promise-based `ConfirmDialog` gates agent deletion (panel button + canvas keyboard via React Flow `onBeforeDelete`) and role-draft Apply overwrite; edge-delete + team-spawn unchanged. 294 tests, opus whole-branch review = ready-to-merge (+1-line `requestConfirm` hardening applied). Audit #8 (Critical) + #31 (Important) resolved. Next recommended cycle: **P1 (crash-safe persistence — atomic writes)**.
